...
## 🔧 Настройка вебхука (требуется настройка)

После успешного деплоя нужно настроить вебхук в Telegram:

1. **Деплой завершен** ✅ - бот запускается с Flask веб-сервером
2. **Настройте вебхук** - выполните команду:
   ```bash
   curl "https://api.telegram.org/botyour_bot_token_here/setWebhook?url=https://telegram-itmo-bot-.onrender.com/webhook"
   ```
3. **Проверьте вебхук** - откройте в браузере:
   ```
   https://telegram-itmo-bot-.onrender.com/webhook
   ```
   Должен появиться ответ: `{"status": "webhook active", "bot_initialized": true}`
...
После настройки вебхука отправьте `/start` боту в Telegram и проверьте логи в Render Dashboard - должен появиться POST запрос к `/webhook`.
