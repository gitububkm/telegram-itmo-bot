...
## 🔧 Настройка вебхука (автоматически)

Бот автоматически настроен для работы с вебхуками на Render:

1. **Деплой завершен** - бот запускается с Flask веб-сервером
2. **Вебхук уже активен** - Telegram отправляет обновления на `/webhook`
3. **URL приложения:** `https://telegram-itmo-bot-.onrender.com/webhook`
...
Если вебхук не работает, проверьте логи в Render Dashboard.
