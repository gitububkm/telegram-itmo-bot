...
2. **SCHEDULE_JSON:** Скопируйте значение переменной `SCHEDULE_JSON` из файла `env_example.txt`
   ...
   **Важно:** Полное расписание находится в файле `env_example.txt` в переменной `SCHEDULE_JSON`. Скопируйте всю строку целиком.

### Шаг 4: Деплой

1. Нажмите "Create Web Service"
2. Дождитесь завершения деплоя (обычно 2-3 минуты)
3. Бот будет запущен автоматически и доступен по URL типа `https://telegram-itmo-bot.onrender.com`

## 🔧 Настройка вебхука (опционально)

Для более надежной работы бота рекомендуется настроить вебхук:

1. В BotFather получите токен бота
2. Установите вебхук командой:
   ```
   https://api.telegram.org/bot<TOKEN>/setWebhook?url=https://your-app-name.onrender.com
   ```
